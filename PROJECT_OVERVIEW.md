...
**Telegram Monitor & n8n Bridge** — это производительный сервис для мониторинга Telegram-каналов через официальный протокол **MTProto (User API)**, интеллектуальной фильтрации контента с помощью LLM (OpenRouter / DeepSeek), прямой пересылки дайджестов через Telegram-бота и оркестрации событий в сценарии **n8n**.
...
* **Гибкая независимая диспетчеризация:** Возможность раздельно включать/выключать:
  ...
  2. Пересылку в **Telegram-бота** (в личку или канал).
  3. AI-обработку через **OpenRouter (DeepSeek / OpenAI / Claude / Gemini)**.
* **Автономный фоновый планировщик:** Периодический опрос каналов с индивидуальными интервалами и лимитами.
...
* **Премиальный SPA UI (60 FPS):** Легкий интерфейс без сторонних тяжелых фреймворков, с микроанимациями **GSAP 3**, поддержкой Markdown-форматирования и мгновенным автосохранением параметров.
...
| **База данных** | SQLite 3 (`storage.db`) | Хранение настроек, каналов, истории сообщений и логов |
...
| **Анимации и микро-UX** | GSAP 3.12.5 | 60fps аппаратные анимации табов, модалок и списков |
...
База данных инициализируется автоматически при старте `server.py` и содержит 5 таблиц:
...
| `offset_hours` | INTEGER | Временное окно выборки в часах (по умолчанию 24) |
...
| `prompt` | TEXT | Индивидуальный промпт для LLM анализа постов этого канала |

### 3. `sent_messages` (История сообщений и дедупликация)
| Поле | Тип | Описание |
|---|---|---|
...
### 5. `integrations_config` (Интеграции с OpenRouter и Telegram Bot)
| Поле | Тип | Описание |
|---|---|---|
...
flowchart TD
    ...
    G -->|Да| H[Генерация ai_analysis через DeepSeek V3]
    G -->|Нет| I[Пропуск AI шага]
    H --> J[Формирование итогового Payload]
    I --> J
...
* `GET /api/telegram-forward` / `POST /api/telegram-forward` — настройки бота для пересылки дайджестов.
...
3. **Настройка n8n Webhook:** Вставьте URL вашего Webhook-узла в n8n.
4. **Настройка OpenRouter (опционально):** Вставьте API-ключ OpenRouter, выберите модель (например `deepseek/deepseek-chat`).
5. **Настройка Telegram-бота (опционально):** Вставьте токен бота из [@BotFather](https://t.me/BotFather) и ID вашего чата/канала.
