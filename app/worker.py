...
# Потолок на СВЕДЕНИЕ, и он намеренно больше. Разбор канала читает посты и
# отвечает данными; сведение пишет итоговое сообщение, и его длина растёт с
# числом находок, а не с числом постов. Одно число на два разных шага и было
# ошибкой: 15 сентября источник из восьми каналов не уложился в 120 с, и в
# бота ушли сырые вердикты вместо оформленного ответа.
#
# Выводится из окна устаревания воркера, а не назначается на глаз: отметка о
# жизни ставится МЕЖДУ единицами работы (см. `_beat`), поэтому вызов длиннее
# окна сделал бы работающего воркера мёртвым на вид — и тревога 13.3 соврала
# бы о простое, которого не было.
REDUCE_TIMEOUT = WORKER_STALE_AFTER - 10.0
...
def _raw_verdicts(verdicts: list[dict]) -> str:
    """Находки без оформления — и честная строка о том, что это они.

    Строку пишет система, а не модель: модель её и не увидит — оформление
    как раз не состоялось. Предупреждение идёт ПЕРВЫМ: поставленное после
    находок, оно читается уже пролистав их (тот же урок, что в 13.1).
    """
    body = "\n\n".join(
        f"{item.get('chat_title')}: {item['verdict']}" for item in verdicts
    )
    return f"⚠️ Оформление не удалось — ниже находки в сыром виде.\n\n{body}"
    ...
    async def _run_source_batch(
        ...
        **Разборы идут последовательно, а не параллельно** — и это
        ...
        for group in groups:
            ...
            # Промпт канала уходит в модель ОДИН, без промпта оформления.
            # Склейка двух промптов в один запрос (была здесь ради экономии
            # на источнике из одного канала) давала модели взаимоисключающие
            # указания: «верни только JSON, оформит следующий шаг» и «оформи
        ...
        if verdicts and template:
            ...
            # а оформлять — есть что, и это разные вопросы.
            ...
            try:
                ...
                # Бюджет месяца кончился — повтор упрётся в тот же потолок, а
                # находки ждать нового периода не должны: они уже добыты и
                # зарезервированы дедупликацией.
                await db.rollback()
                await add_log(
                    db,
                    user_id,
                    "AI_ERROR",
                    "Сведение не удалось: месячный бюджет токенов исчерпан",
                    status="ERROR",
                    chat_title=batch.get("chat_title"),
                )
                analysis = _raw_verdicts(verdicts)
            except Exception as exc:  # noqa: BLE001 — находки дороже оформления
                await db.rollback()
                if attempts + 1 < MAX_BATCH_ATTEMPTS:
                    # Отказ оформления восстановим, и повтор дёшев: вердикты
                    # каналов уже сохранены в задаче, переспрошено будет
                    # ТОЛЬКО оформление. Отдавать сырые вердикты с первой
                    # попытки значит заплатить за разбор и выбросить
                    # оформление из-за одной медленной генерации.
                    await add_log(
                        db,
                        user_id,
                        "AI_ERROR",
                        f"Сведение не удалось, батч ждёт повтора: "
                        f"{redact(_reason(exc))}",
                        status="ERROR",
                        chat_title=batch.get("chat_title"),
                    )
                    raise
                ...
                # Сырые вердикты лучше потерянных находок — но человек обязан
                # узнать, что перед ним не оформленный ответ: молчаливая
                # подмена отправляет искать ошибку в промпте, которого она не
                # касается (найдено владельцем 15 сентября).
                analysis = _raw_verdicts(verdicts)
