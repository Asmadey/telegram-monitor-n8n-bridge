...
style-src 'unsafe-inline' СНЯТ 2026-09-15. Он стоял не по недосмотру:
страницы входа держали инлайн-СТИЛИ, и комментарий здесь честно называл
причину. Причины больше нет — уборка дизайн-системы вынесла блоки `<style>`
экранов входа в `auth.css`, а все 199 атрибутов `style=` разметки и модулей
в классы. Разрешение пережило бы свою причину, а это ровно тот класс
дефектов, который проект ловит всю неделю.

Присваивания `element.style.x` из скриптов под запрет не попадают: CSSOM
политика не трогает, переключения видимости работают как работали.

img-src с data: — аватарки-заглушки и встраиваемые картинки, исполнить код
через data:-картинку нельзя.
"""
