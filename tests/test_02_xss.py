...
# `esc(` убран задачей 12.3 вместе с самой функцией: маркер живёт дольше кода,
# и сканер благословлял бы вызов, которого в render.js уже нет, — то есть
# признавал бы безопасным ReferenceError.
ESCAPING_CALLS = (
    "escapeHtml(",
    "formatTelegramText(",
    "highlightText(",
...
SAFE_SINKS = {
    ...
    # Диалоги браузера показывают ТЕКСТ: разметка в них не разбирается.
    # Добавлены 2026-09-14, и добавлены по делу: `confirm()` в
    # `deleteSource` подставляет имя источника с первого дня, а свип молчал
    # — ближайшим маркером в окне случайно оказывался `.textContent` из
    # соседней функции. Вставка новой функции между ними сдвинула окно, и
    # строка «покраснела» без единой правки в ней самой. То есть зелёным
    # этот случай был не потому, что разобран, а потому, что повезло.
    "confirm(": "диалог браузера показывает текст, разметка не разбирается",
    "prompt(": "то же самое: поле ввода диалога, а не документ",
    "alert(": "то же самое",
}

SCRIPT_BLOCK = re.compile(r"<script\b[^>]*>(.*?)</script>", re.S | re.I)
INTERPOLATION = re.compile(r"\$\{([^{}]*(?:\{[^{}]*\}[^{}]*)*)\}", re.S)
# `(?<!-)` — имя, которому предшествует дефис, это ЧАСТЬ имени
# CSS-переменной, а не обращение к полю: `var(--text-caption)` содержит
# `text`, `--surface-bad` содержал бы `status`. Поймано 2026-09-15, когда
# шкала текста получила имена `--text-*`: сканер потребовал экранировать
# статическую галочку `✓`, в которой данных нет вовсе.
#
# Лечится сканер, а не имена токенов. Переименовать шкалу значило бы
# подогнать код под изъян проверки — и оставить его ждать следующего, кто
# назовёт переменную `--status-…` или `--type-…`.
