...
def test_the_notice_names_the_scope_of_access():
    """Главное, что человек отдаёт, — доступ на чтение переписки."""
...
def test_the_account_risk_is_named_at_least_in_the_consent():
    """Решение владельца 2026-09-15: абзац про правила Telegram убран.

    Прежняя проверка требовала слова «ограничить» и после удаления абзаца
    осталась ЗЕЛЁНОЙ — но уже по другой строке: «ограничить доступ только
    нужными каналами нельзя» говорит об ОБЪЁМЕ доступа, а не о риске для
    аккаунта. Слово совпало, смысл разошёлся; это ровно тот ложный зелёный,
    который проект ловит с фазы 0, и найден он был не прогоном, а глазами.

    Что осталось в интерфейсе после решения владельца — строка согласия.
    Её и проверяем: риск для аккаунта **назван**, но больше нигде не
    объяснён — человек не прочитает, что аккаунт могут ограничить или
    заблокировать и что решает это Telegram, а не сервис. Проверка обязана
    говорить правду о том, что осталось, а не хранить память о том, что было.
    """
    step = _step_phone().lower()
    start = step.find("risk-consent")
    assert start != -1, "строки согласия нет вовсе — риск не назван нигде"
    assert "риск для аккаунта" in step[start:], (
        "в согласии не назван риск для аккаунта, а объяснения риска в шаге "
        "больше нет: человек соглашается неизвестно с чем"
    )
