...
app.main / app.worker. Монолит (~40 эндпоинтов без auth, К2) удалён задачей
7.4 и запрещён к возвращению (test_75) — проверки ниже остаются заслоном на
случай возврата: образ не должен УМЕТЬ поднять незакрытую сборку, даже если
её кто-то вернёт в дерево. Поэтому CMD Dockerfile — часть безопасности, а не
деплойная мелочь, и тест на него — статический трипваер.

Здесь проверяются КОНФИГИ в репозитории, а не живые сервисы. Поведенческий уровень (редеплой
...
def test_docker_image_runs_new_assembly_never_monolith():
    ...
    (монолит без auth, К2, удалён задачей 7.4 — заслон на случай возврата)."""
...
def test_procfile_never_launches_the_monolith():
    ...
    Его читают foreman/hivemind, Heroku и Railway с NIXPACKS-билдером.
    Появись в нём `server:app`, утверждение «монолит не запустим даже
    случайно» стало бы неверным: это и есть путь «случайно». Сам монолит
    удалён задачей 7.4 — проверка сторожит возврат.
    """
    path = ROOT / "Procfile"
    if not path.exists():
        return
...
def test_monolith_source_is_not_shipped_in_the_image():
    """Глубина защиты: `docker run <image> uvicorn server:app` поднял бы
    незакрытую сборку даже при правильном CMD. Монолит удалён задачей 7.4,
    правило `.dockerignore` оставлено заслоном на случай возврата: COPY . .
    забирает всё, что окажется в контексте, и стоит правило одну строку.
    """
    patterns = {
        line.strip()
