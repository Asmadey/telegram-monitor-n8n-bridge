...
async def test_webhook_saved_and_masked(anon_client, db, user, allow_webhook):
    ...
    assert body["webhook_url_masked"], "маска пропала из ответа"

    # Здесь стояло `GOOD_WEBHOOK not in str(body)` — «адрес не уходит наружу
    # целиком». Утверждение изменено осознанно (задача 9.14, требование
    # владельца), и вот почему оно было слишком широким.
    #
    # Правило 0.3 (К3) родилось в монолите, где ЭТОТ ЖЕ ответ уходил
    # анониму: аутентификации не было ни на одном эндпоинте. Опасность была
    # не в том, что адрес показан хозяину, а в том, что он показан кому
    # угодно. Сегодня ответ скоуплен `TenantRepo` и закрыт входом.
    #
    # А адрес вебхука — конфигурация, а не удостоверение: его задаёт сам
    # пользователь и сам же отзывает в n8n. Скрытый, он делал поле
    # неработающим: открыв вкладку, владелец видел пустую строку и не мог
    # ни проверить адрес, ни исправить опечатку в нём.
    #
    # Что осталось непоколебимым и проверяется тут же: ключ и токен —
    # удостоверения — в этот ответ по-прежнему не попадают, а чужой адрес
    # не виден никому (`test_103_secret_visibility.py`).
    assert body["webhook_url"] == GOOD_WEBHOOK, (
        "владелец не видит собственный адрес вебхука — поле нечем заполнить"
    )
    assert "api_key" not in body and "bot_token" not in body, (
        "в ответ вебхука затекли удостоверения соседних интеграций"
    )
