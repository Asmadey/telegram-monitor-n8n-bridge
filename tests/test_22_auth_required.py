...
# Адреса вкладок SPA (К2): отдают ту же оболочку, что и "/", — HTML без единого
# байта данных тенанта. Данные приходят из /api/*, закрытых require_user, а сам
# SPA уводит анонима на /login. Отдавать здесь 401 в ответ на переход по ссылке
# значило бы показывать пользователю JSON вместо страницы входа.
PUBLIC_EXACT |= {"/feed", "/channels", "/messages", "/integration", "/logs"}
