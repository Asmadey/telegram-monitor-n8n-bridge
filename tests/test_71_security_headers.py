...
Инлайн-СТИЛИ держались на страницах входа (5.3) и требовали
'unsafe-inline'. 2026-09-15 уборка дизайн-системы вынесла их в `auth.css`, а
все 199 атрибутов `style=` — в классы; разрешение снято. План требовал
запретить именно СКРИПТЫ, стили не
исполняют код.
...
async def test_csp_allows_self_for_style_img_connect(anon_client):
    ...
    **Требование перевёрнуто 2026-09-15.** Раньше этот тест ТРЕБОВАЛ
    'unsafe-inline' в style-src — и был прав: страницы входа держали
    инлайн-стили, без разрешения они бы сломались. Уборка дизайн-системы
    вынесла их в `auth.css`, а все 199 атрибутов `style=` — в классы.
    Причина исчезла, и разрешение, пережившее свою причину, — ровно тот класс
    дефектов, который проект ловит всю неделю. Теперь тест требует обратного.
    """
    resp = await anon_client.get("/")
    csp = _csp(resp)
    style_directive = (
    ...
    assert "'unsafe-inline'" not in style_directive, (
        "style-src снова разрешает инлайновые стили: причина (инлайн-стили "
        "страниц входа) устранена 2026-09-15, разрешение должно было уйти с ней"
    )
