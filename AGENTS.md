...
- **`app/api/auth.py` и `app/api/telegram.py`** — доавторизационные пути: сессия, попытка входа,
  привязка Telegram-аккаунта. Это собственные записи пользователя, а не тенантные ресурсы, и
  репозитория в этот момент ещё не существует.
