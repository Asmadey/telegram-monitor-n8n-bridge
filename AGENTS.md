...
фильтр невозможно: `repo.query(Model)` для чтения, `repo.delete(Model)` для очистки. Один забытый
`where` в одном обработчике из сорока — и пользователь A читает ленту пользователя B; забытый
`where` в удалении стирает данные всем клиентам сразу.

**Исключение — и оно ровно одно: репозитория на руках нет.** Так устроены два семейства мест:

- **`app/services/`** — службы получают `user_id` аргументом. Их зовёт воркер, где нет ни запроса,
  ни зависимости `get_tenant_repo`, ни репозитория.
- **`app/api/auth.py`, `app/api/telegram.py`, `app/services/tg_session.py`** — собственные записи
  пользователя: сессия, попытка входа, привязка Telegram-аккаунта и ключи его приложения. Это не
  тенантные ресурсы, и репозитория в этот момент ещё не существует.

Исключение сформулировано так, чтобы его нельзя было растянуть: **есть `repo` в функции — ручной
фильтр запрещён**, независимо от файла. Это свойство кода, а не список, и `test_124` проверяет
именно его; список файлов из второго вида закреплён там же, чтобы не рос молча.
